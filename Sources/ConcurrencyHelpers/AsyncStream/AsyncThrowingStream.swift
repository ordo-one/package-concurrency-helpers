...
public struct AsyncThrowingStreamBP<Element, Failure: Error> {
...
@available(SwiftStdlib 5.1, *)
extension AsyncThrowingStreamBP: AsyncSequence {
...
@available(SwiftStdlib 5.1, *)
extension AsyncThrowingStreamBP.Continuation {
  /// Resume the task awaiting the next iteration point by having it return
...
extension AsyncThrowingStreamBP {
  ...
  public static func makeStream(
      ...
      bufferingPolicy limit: Continuation.BufferingPolicy = .unbounded
  ) -> (stream: AsyncThrowingStreamBP<Element, Failure>, continuation: AsyncThrowingStreamBP<Element, Failure>.Continuation) where Failure == Error {
    var continuation: AsyncThrowingStreamBP<Element, Failure>.Continuation!
    let stream = AsyncThrowingStreamBP<Element, Failure>(bufferingPolicy: limit) { continuation = $0 }
    return (stream: stream, continuation: continuation!)
  }
}

@available(SwiftStdlib 5.1, *)
extension AsyncThrowingStreamBP: @unchecked Sendable where Element: Sendable { }
#else
@available(SwiftStdlib 5.1, *)
